...
static const uintptr_t ModuleBase = reinterpret_cast<uintptr_t>(GetModuleHandleA(NULL));

constexpr auto rebase = [](auto inPtr) {
    const uintptr_t runtimeAddr = reinterpret_cast<uintptr_t>(inPtr);
    const uintptr_t rva = runtimeAddr - ModuleBase;
    return rva;
};
...
void IdaExporter::ExportDeclarations(const RTTI &inType) {
    ...
    fprintf(mFile, "\n\t// %s %s\n",kind_name.c_str(), type_name.c_str());
    ...
    for (const auto &symbol: inGroup.mSymbols) {
        ...
        for (const auto &language: symbol.mLanguage) {
            ...
            if (symbol.mKind == ExportedSymbol::Kind::Function) {
                ...
                fprintf(mFile, "\tset_func_cmt(%#llx, \"%s%s", rebase(language.mAddress), signature[0].mName, strlen(signature[0].mModifiers.c_str()) ? signature[0].mModifiers : " ");
                if (symbol.mNamespace)
