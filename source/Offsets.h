...
namespace Offsets {
    ...
    std::pair<uintptr_t, uintptr_t> GetModuleSection(std::string_view Section);
    ...
    void MapAddress(std::string_view ID, uintptr_t Offset);

    void MapSignature(std::string_view ID, std::string_view Signature, int Adjustment = 0);

    uintptr_t FindOffset(std::string_view ID);
    ...
    struct LiteralHash {
        ...
        static constexpr uint64_t FNV1A(std::string_view Buffer) {
