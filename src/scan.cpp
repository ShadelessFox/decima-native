...
bool FindSection(void *module, const char *name, struct Section *result) {
...
static int ScanPattern(uint8_t *start, const uint8_t *end, const char *pattern, uint8_t **position) {
    while (start < end && *pattern) {
        if (*pattern == '?') {
            pattern += 2;
        } else if (*start == strtol(pattern, nullptr, 16)) {
            pattern += 3;
        } else {
            *position = start + 1;
            return 0;
...
bool FindPattern(void *start, const void *end, const char *pattern, void **position) {
    void *current = start;
    while (current < end) {
        if (ScanPattern((uint8_t *) current, (uint8_t *) end, pattern, (uint8_t **) &current)) {
            *position = current;
            return true;
        }
    }
